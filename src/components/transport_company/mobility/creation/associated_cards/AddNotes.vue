...
<template>
      ...
      <q-card-section class="q-py-sm" style="background-color: var(--fleet-color-gray-true-200)">
        <div class="row items-center">
          <div class="col-8 flex items-center">
            <iconify-icon icon="hugeicons:note" width="24px" height="24px" class="text-grey-7" />
  ...
  <!-- Event Note Drawer -->
  <q-dialog persistent backdrop-filter="blur(2px)" v-model="eventNoteDrawer" position="right" :full-height="true">
    <q-card style="width: 500px" class="column">
      <q-card-section class="q-pb-none">
        <div class="row items-start justify-between">
          <div>
            <div class="text-h6 text-weight-bold">Event Note</div>
            <div class="text-caption text-grey-7">Add a general note or comment to this booking.</div>
          </div>
          <div class="row items-center q-gutter-x-md">
            <q-btn color="primary" label="Apply" @click="handleApply" />
            <q-btn flat round dense icon="close" @click="eventNoteDrawer = false" class="text-grey-7">
